...
const DynamicListSms = (props) => {
    ...
    const setDefaultActiveItem = () => {
        let element = document.querySelector(`${"#sendsms"}`);
        element.classList.add("border-blue-700", "border-l-4", "bg-blue-50");
    };

    const activePanel = (item) => {
        let listItems = document.getElementById('drawer-item-container-sms');
...
export default withRouter(DynamicListSms);
